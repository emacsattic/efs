;;; efs.el --- Transparent FTP support
...
;;;    |$Date: 2002/08/27 14:56:23 $|$efs release: 1.24 $|
...
(defconst efs-month-regexp-alist
  '(("\\([jJiI]aa?n\\|ene\\|Gen\\)" . 1)
    ("\\([FfvV][eé]e?[bv]\\|veebr\\)" . 2)
    ("\\([Mm][aä]r\\|märts\\|mrt\\)" . 3)
    ("\\([Aa][bpv]r\\)" . 4)
    ("\\([Mm][aáe][íijy]\\|Mag\\)" . 5)
    ("\\([JjIi][uú]n\\|juuni\\|jui\\|Giu\\)" . 6)
    ("\\([JjIi][uú]l\\|juuli\\|Lug\\)" . 7)
    ("\\([Aa][uv]g\\|ago\\|aoû\\|ágú\\|Ago\\)" . 8)
    ("\\([Ss]ep\\|sept\\|Set\\)" . 9)
    ("\\([Oo][ck]t\\|Ott\\|Out\\)" . 10)
    ("\\([Nn][oó]v\\|Noi\\)" . 11)
    ("\\([Dd][eé][scz]\\|dets\\|dic\\|Dic\\)" . 12)))
...
by sending an enquiry to elisp-code-efs@nongnu.org.
